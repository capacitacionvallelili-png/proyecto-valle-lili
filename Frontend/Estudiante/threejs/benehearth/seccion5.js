...
const PARTES = [
...

    {
        ...
        pasos: [
            { objeto: 'Carga', imagen: '/Estudiante/threejs/img/manual.png', instruccion: 'Puedes hacer la carga ya sea desde las palas, o desde el panel frontal, en este caso hazla desde el panel frontal' },
    ...
    {
        id: 'IMPRESORA',
        nombre: 'Al finalizar se imprimirá todo lo que obtenga sobre el código, es importante guardar el papel y dejar que imprima por completo',
        tipo: 'click',
        instruccion: "Dale click a la impresora, y guarda el papel al finalizar",
        objeto: 'IMPRESORA',
        //camaraOffset: { x: 0, y: 0, z: 0.3 },
    },
];
...
/* =====================================================
   ...
   targetPctX / targetPctY = centro de la zona correcta
   expresado como porcentaje del ancho y alto de la imagen.

   Posiciones según la imagen de referencia proporcionada:
   - Esternón (pala derecha): zona superior derecha del pecho
   - Ápex (pala izquierda): zona inferior izquierda, bajo el pecho

   Ajusta estos valores si necesitas afinar la posición:
     targetPctX = (left_centro / img.offsetWidth)  * 100
     targetPctY = (top_centro  / img.offsetHeight) * 100
   ===================================================== */
const PALAS_CONFIG = [
    {
        id: '1',
...
function onMouseUp() {
    ...
    if (parte.anguloObjetivo && objRotado) {
        ...
        if (!modoObjetivo) {
            ...
            if (diferencia > TOLERANCIA_ANGULO_PERILLA) {
                mostrarToastPerilla(`Gira hacia desfribilación "${parte.anguloObjetivo}"`, false);
    ...
    mostrarToastPerilla(
        parte.anguloObjetivo ? `¡Modo Desfribilación ${parte.anguloObjetivo} alcanzado!` : '¡Listo!',
        true
    );
    document.getElementById('feedbackPerillaS5')?.remove();
    reproducirSonidoCorrecto();
