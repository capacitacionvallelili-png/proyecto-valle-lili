...
const PARTES = [
    ...
    {
        id: 'rosca',
        nombre: 'Para la prueba de electrodos con energía debes cambiar el cable de las palas',
        objeto: 'rosca',
...
export function destruirSeccion3() {
    ...
    document.getElementById('uiSeccion3')?.remove();
    document.getElementById('loaderSeccion3')?.remove();
    document.getElementById('checklistS3')?.remove();
...
function ocultarLoader() {
    const loader = document.getElementById('loaderSeccion3');
    if (!loader) return;
    ...
    ui.innerHTML = `
        <div id="checklistS3" style="
            ...
            <p style="font-size:0.78rem;font-weight:600;color:#1e5c3a;
                ...
                Pasos para la Prueba de usuario
            </p>
            ${PARTES.map((parte, i) => `
        ...
        <div id="panelLateralS3" style="
            ...
            <div style="display:flex;align-items:center;
                ...
                <h4 style="font-size:1rem;color:#1e5c3a;margin:0;">Prueba de usuario</h4>
                <button onclick="window.togglePanelS3()" style="
                    background:none;border:none;cursor:pointer;
                    color:#5a7a62;font-size:1.2rem;padding:4px;">✕</button>
            </div>
            <p style="margin:0 0 12px;">
                La prueba de usuario es una verificación diaria del estado físico y funcional
                del equipo. Comprueba que todos sus componentes operativos están en condiciones
                correctas: botonería del panel, palas adulto/pediátricas, pantalla, impresora y
                batería. Es importante porque detecta fallas antes de que el equipo sea requerido
                en una emergencia real.
            </p>
        </div>

        <div id="overlayPanelS3" onclick="window.togglePanelS3()" style="
...
function marcarTodoCompletado() {
    ...
    if (instruccion) instruccion.textContent = 'Prueba de usuario completada, selecciona de nuevo un paso de la lista para recordar';
    document.getElementById('feedbackPerillaS3')?.remove();
}
...
window.irAParteS3 = function (indice) {
...
function actualizarFeedbackPerilla(modoActualNombre) {
    const el = document.getElementById('feedbackModoActual');
...
window.togglePanelS3 = function () {
    const panel   = document.getElementById('panelLateralS3');
...
function mostrarToastPerilla(mensaje, exito) {
    document.getElementById('toastPerillaS3')?.remove();
    const contenedor = document.getElementById('areaThreeJs');
    const toast = document.createElement('div');
    toast.id = 'toastPerillaS3';
    toast.style.cssText = `
...
function mostrarVideoPopup(parte, onTerminado) {
    ...
    setTimeout(() => iniciarYTPopupS3(onTerminado), 600);
}
...
function iniciarYTPopupS3(onTerminado) {
            ...
            events: {
                onStateChange: e => {
                    if (e.data === YT.PlayerState.ENDED) {
                        document.getElementById('avisoPopupS3')?.remove();
                        setTimeout(() => {
                            cerrarVideoPopup();
                            if (ytPlayer) { try { ytPlayer.destroy(); } catch { } ytPlayer = null; }
                            onTerminado?.();
